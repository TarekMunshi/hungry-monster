...
const submitUserInput = () => {
    ...
    document.getElementById("ingredientWrapper").style.display = "none";
    if (inputValue !== '') {
        document.getElementById("warningText").innerText = "";
        getAllMealsData(inputValue);
    } else {
        document.getElementById("warningText").innerText = "Please input a value first.";
...
const getSingleMealDetails = (mealId) => {
    fetch(`https://www.themealdb.com/api/json/v1/1/lookup.php?i=${mealId}`)
        ...
        .catch(error => {
            const parentDiv = document.getElementById("ingredientWrapper");
            showingErrorMsgToUser(parentDiv, "There are no products with the searched input. Try another one.");
        });
};
...
const displaySingleMealData = meal => {
    ...
    const ingredientParentDiv = document.getElementById("ingredientWrapper");
